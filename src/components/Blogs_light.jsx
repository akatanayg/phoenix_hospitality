...
const BlogsCarousel = () => {
  ...
  return (
    <section
      className={`w-full py-12 px-4 sm:px-6 md:px-16 transition-colors duration-300 ${
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-10">
        ...
        <div className="space-y-6 text-center md:text-left">
          ...
          >
            ...
            <p
              className={`text-[16px] sm:text-[20px] md:text-[22px] font-medium leading-relaxed ${
                darkMode ? "text-red-400" : "text-[#E63946]"
              }`}
            >
              Dive into stories, experiences, and inspirations from our heritage
              home.
            </p>
          </motion.div>
          ...
          <div className="pt-4">
            <a
              href="/blogs"
              className={`inline-block py-3 px-6 rounded-full font-medium tracking-wide transition duration-300 ${
                darkMode
        ...
        <div className="relative mt-6 md:mt-8">
          ...
          >
            <div
              ...
              style={{
                transform: `translateX(-${currentIndex * 50}%)`, // Moves two slides at a time
                width: `${(blogs.length / 2) * 100}%`, // Ensures proper width calculation
              }}
            >
              {blogs.map((blog, index) => (
                <motion.div
                  key={index}
                  className="relative w-1/2 flex-shrink-0 h-[350px] sm:h-[400px] rounded-xl shadow-lg overflow-hidden cursor-pointer group"
                >
                  {blog.image && blog.title ? (
                    <>
                      ...
                      <div className="absolute inset-0 bg-black bg-opacity-40 flex flex-col justify-end p-4 sm:p-5 transition-all duration-500 group-hover:bg-opacity-60">
                        ...
                        <span className="text-white text-[10px] sm:text-xs mt-2 opacity-60">
                          {blog.date}
                        </span>
                      </div>
                    </>
